...
export class Bulletin extends Component {
    ...
    private async showBulletin() {
        ...
        // await delay(3, this.node);
        if (!isVaild(this.node)) return;
        this.bulletin.x = 1200;
        const p = "Player_" + MathUtil.random(100000, 999999);
        const money = LangStorage.getData().symbol + " " + WithdrawUtil.moneyCash.getRandomItem();
        this.text.string = "<b><outline color=#333333 width=1>" + i18n.string("str_bulletin", p, money);
